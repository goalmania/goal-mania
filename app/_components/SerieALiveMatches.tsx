...
async function fetchLiveMatches(): Promise<{ matches: Match[]; error?: string }> {
  ...
  try {
    ...
    const serieAMatches = matches.filter(
      (match: Match) => match.league.name === "Serie A"
    );

    if (serieAMatches.length > 0) {
      return { matches: serieAMatches };
    } else {
      return { 
        matches: mockMatches, 
        error: "No live Serie A matches currently. Showing recent matches instead." 
      };
    }
  } catch (error) {
...
export default async function SerieALiveMatches() {
